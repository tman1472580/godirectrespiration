...
class LivePlotter:
    def __init__(self, window_size):
        ...
        self.fig.suptitle('Respiration Belt + Heart Rate → MIDI (Breath-Adaptive Calibration)')
...
def connect_device():
    """Connect to Go Direct device synchronously (like the working code)."""
    g = gdx.gdx()
...
def main():
    # Connect to respiration belt (synchronous, like working code)
    g = connect_device()
    if g is None:
    ...
    # Display starting metrics - wait for stable readings
    start_breath_rate = None
    start_hr = hr_monitor.get_start_hr()
    breath_rate_captured = False
    min_breaths_for_start = 5  # Wait for at least 3 breaths before capturing start rate
    ...
    print(f"Breathing Rate: Detecting (waiting for {min_breaths_for_start} stable breaths)...")
    print("="*60 + "\n")
    ...
    try:
        while True:
            ...
            if event:
                ...
                current_breath_rate = norm.get_bpm()
                
                # Capture start breath rate only after enough stable breaths
                if not breath_rate_captured and current_breath_rate and norm.total_breaths >= min_breaths_for_start:
                    # Also check that the rate is reasonable (2-60 BrPM)
                    if 2.0 <= current_breath_rate <= 60.0:
                        start_breath_rate = current_breath_rate
                        breath_rate_captured = True
                ...
                if in_min is not None and in_max is not None:
                    print(f"New breath range → [{in_min:.5f}, {in_max:.5f}] | "
                          f"Breathing: {(current_breath_rate or 0):.1f} BrPM | "
                          f"Heart: {(hr_monitor.get_current_hr() or 0):.1f} BPM | "
            ...
            if frame % 10 == 0:
                bpm = norm.get_bpm()
                info = (
                    f"Breathing Rate: {bpm:.1f} BrPM" if bpm else "Breathing Rate: detecting..."
                )
        ...
        if start_breath_rate and end_breath_rate:
            ...
            print(f"\nBreathing Rate Change: Not available (start rate not captured)")
        print("="*60 + "\n")
        ...
        if hr_monitor.polar:
            ...
            # Note: cleanup happens in daemon thread
        plt.close('all')
        print("Goodbye.")
